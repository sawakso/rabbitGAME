...
| 层 | 机制 | 作用 |
| --- | --- | --- |
| 产出 | 兔子在家种胡萝卜（离线上限 8 小时）+ **采胡萝卜关卡**挣胡萝卜 | 留存 / 时长 |
...
**「采胡萝卜」是关卡制**（照开心消消乐的成熟路数）：

- **每关一个目标**：采 N 个指定萝卜，限步内完成。目标种子固定——
  **同一关每次进来棋盘布局一模一样**，输了能复盘，这是三消关卡手感的基础
- **难度平滑递增**：第 1 关 6 个（一遍必过建立信心），之后每关 +~1，
  步数在高关卡小步放宽；**每 15 关一个新层次**，过关额外 +100
- **星级**：按剩余步数算 1~3 星，**每剩 1 步折 2 个胡萝卜**当奖励
- **奖励结构**：过关奖 = 15 + 10×星级 + 剩余步数折算；
  **每 5 关送一张必出稀有的盲盒券**（和抽衣橱直接挂钩）
- **失败是软的**：步数用完只说「就差 X 个…」，采到的胡萝卜照常结清，
  重试免费重开同一布局（只扣 1 点体力）；「看广告 +5 步」还是「我还想再消一会儿」，
  不是「我输了要救」
- 顶部第三格显示**第 N 关 · 目标**（带目标萝卜的小图标和进度）
...
- 棋子是「多汁」卡通风：径向渐变体积 + 左上高光 + 深色描边，全部代码画
  （参照水果消除类爆款的美术路数——那类画面不是 3D，是画出来的体积感）
